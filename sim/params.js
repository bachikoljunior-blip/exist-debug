...
module.exports = {
  ...
  quota: {
    ...
    // 層の試練(trialCoef/trialStartLayer/trialFloorRuns)は削除(2026-07-27 ユーザー指示「ノルマは経過秒でのみ変化」)。
    // 2026-07-11 の「ノルマ層でノルマ加速するのやめて」で coef=0 の休眠状態だった機構を、式ごと sim/ゲームから撤去した。
    // 掃引スクリプト sweep_trial.js も対象機構ごと廃止(先頭で廃止を表示)。
    // ---- ノルマ終了(第12次H・提案9 → 2026-07-16 ⑧確定形 → 2026-07-27 時間比の完全撤去) ----
    // 未達は「転生の一歩手前」で発火する。時間比(ρ=周回内経過秒 ÷ 前回周回の長さ)は**撤去済み**
    // (2026-07-27 ユーザー指示「ノルマ前回周回長に依存するのやめて」)。
    // reachCoef/reachPow/reachMinSec/reachMaxSec/reachEmaAlpha と sim.prevDuration も同時に削除=
    // ノルマの終了時刻は前回の周回長に一切依存しない(履歴非依存)。掃引スクリプト sweep_reach.js /
    // sweep_maxsec.js は対象機構ごと廃止(記録として残置・実行しても何も変わらない旨を先頭で表示)。
    ...
    // 実停止は転生インターセプト(1秒)のみ。0にすると未達が一切起きない(旧・時間比のフォールバックは無い)。
    // ゲーム(index.html)側は同じ「転生の一歩手前」を戦略なしで表現する: 経過 ≥ 1176秒(=1200×0.98)かつ
    // 獲得予定PT ≥ 次に取れる最安スキル × 0.983(QUOTA_END_* / cheapestNextSkillCost)。
  },
