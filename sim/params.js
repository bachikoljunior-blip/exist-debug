...
module.exports = {
  ...
  monster: {
    ...
    // ㉚系・面白さ①(2026-07-21 R33 ユーザー激怒「モンスターがワンタップで消える」): 最小タップ床。
    // 1タップの削り量を maxHp/minTaps に制限=どんな高火力でも必ず minTaps 回叩かないと倒せない
    // (HP式・火力式は不変。ワンパンだけを禁じる)。オーバーキル判定は生ダメージのまま=素材機構は無傷。
    // 出現間隔42-80秒に対しttk=minTaps/tapRateはコンマ秒なのでkillsPerSec(討伐系③⑫㉘)は1-2%しか動かない。
    // 型ごとに手応えを変える(通常4・群れ2=数で押す・俊足3・戦車8・ボス16・黄金獣6)。
    minTaps: { normal: 4, swarm: 2, speedy: 3, tank: 8, boss: 16, goldenBeast: 6, default: 4 }
  },
  ...
  res: {
    ...
    portalHuntSpawn: 0.0008, portalHuntSpawnBase: 0.007, // R34実験を差し戻し(2026-07-21): 0.005に上げても⑬延長狩りは全方針1.000のまま=真因は窓中加速の弱さでなく測定構造(⑬snapが段2取得直後=周回末期t82hで取られ900s窓にモンスター希薄→両枝同一化)。検証不能な便益+③⑫㉘への波及リスク=do no harmで元値へ。⑬延長狩りの測定は2026-07-26に決着: snapを実効地点へ移した上で、総クッキー比では原理的に測れない(sim実収入に討伐パルス無し)と確定し、判定を「狩り窓の稼働秒」=機構量へ変更(runner.js MECH)。窓中加速の係数はこの値のまま据え置き。
  ...
  res2: {
    // 上位設備の指数増幅研究7件の投資量への重み(2026-07-27 OPEN1の再設計・1=台数そのまま)
    upperAmpWeight: 1,
