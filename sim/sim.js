...
function advanceTick(sim, strategy) {
  const dt = 1;
  {
    ...
    // ㉚バッチ開発の発火判定: 進行量アンカー(前周回総生産の5%/60%/8倍)を越えるたび1回ずつバッチtick。
    // 1点だけだと未購入の初開発が最大1周回待ち=効果の到着遅れで全方針が保険転生の匍匐(+0.5-1桁/周回)に
    // 落ちる(v20実測: S2が100周回で全解放不能)。3点なら待ちは最大約1/3周回=成長が戻る。
    // 各バッチ間は進行量で1.1〜2.2桁離れており、モーメント間隔は通常数分(熱い周回で近接する場合は
    // 空バッチ=イベント無しが大半)。
    {
      const BP = [0.05, 0.32]; // 上位アンカー(8×/40×)は撤去(2026-07-25 ④構造保証後): 熱い周回で転生(スキル束)の直前に発火しΔ27sを作る(S17実測)。転生点は常に≥前周回超+gain目標=1.6×より十分上にあり、1.6×までのアンカーは転生と構造的に離れる。1.6×以遠で越えた項目は翌周回の0.05/0.32/1.6アンカー(財布が前周回総生産の3-60%=旧帯の10-190倍)が全カバー
...
// ㉚バッチ開発モデル(2026-07-25 ユーザー最終指示「30秒だけ完全達成・他は致命傷なければ可」):
// 生涯初の開発(=解放イベント)は「周回に1回のまとめ開発」でのみ行う。発火点=その周回の総生産が
// 前周回の50%に達した瞬間(進行量ゲート=時間・収入との連動なし)。この1tickに買える初開発を
// 同tickチェックアウトで買い切る=ユーザー規則「同一tick=1モーメント=1判定」により1回の解放体験。
// これで解放モーメントは構造的に「開発バッチ(周回中盤)+スキル束(転生時)」の周回2個となり、
// 相互の間隔は周回の前半/後半そのもの(数分〜)=㉚が配置によらず成立する。
// 買えなかった初開発は次周回のバッチへ持ち越し(前周回に財布が越えた段は翌周回のバッチで必ず買える:
// バッチ時財布≈前周回総生産の3-5割 ≫ 前周回中に越えた全コスト)。run0のみ従来の到達即購入
// (前周回が無く、素の低速経済=間隔は経済が作る)。プレイヤー語: 「再建が済んだら新開発をまとめて行い、
// 終盤は転生に備える」。
function prestigeImminent(sim) {
  if (sim._immiT === sim.t) return sim._immiV; // 毎tickキャッシュ(tryBuy*全部から呼ばれるため)
  sim._immiT = sim.t;
  if (!(sim._prevRC > 0) || process.env.NOBATCH === '1') return (sim._immiV = false); // run0: 従来挙動(到達即購入)/ NOBATCH=A/B診断用
  return (sim._immiV = sim._batchTick !== sim.t); // バッチtick以外は初開発禁止
}
