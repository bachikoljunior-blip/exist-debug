...
(async()=>{
  ...
  // 逆に稼働率を一律2%にすると、序盤2分で13タップ=最初の1台(強い指185クッキー・2026-07-27のsim同期前は100)も買えず動かない(これも実走で確認)。
  ...
  const stepActions=async(tapN,bank)=>withTO('stepActions(討伐/報酬/金/研究/買い物/タップ)',()=>p.evaluate(({tapN,bank})=>{
    ...
    let boosted=false;
    try{ boosted=(typeof goldenBoostActive==='function')&&goldenBoostActive(); }catch(e){}
    const taps=Math.round(tapN*(boosted?1.5:1));
    if(tapCookie&&taps>0){for(let k=0;k<taps;k++)tapCookie();add(boosted?'金ブースト中に集中タップ':'クッキーをタップ',taps);}
    return out;
  },{tapN,bank}));

  // 工房の実プレイヤー行動は workshop_policy.js の共有ルール(空き枠を埋める・実測で良くなるときだけ付け替え・料理)。
  const workshopActions=async()=>withTO('工房(装備作成/装着/料理)',()=>p.evaluate(()=>(window.__workshopActions?window.__workshopActions():[])));
