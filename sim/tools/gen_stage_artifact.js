...
${entries}
  ...
  <footer class="foot">序盤の芯の輪（タップ→討伐→報酬→金→設備）で毎秒生産を立ち上げ、<strong>放置</strong>で厚く貯めてから<strong>モンスターを狩る</strong>——これを転生で繰り返す。討伐数は転生を跨いで貯まり、<strong>累計100体でクエスト達成＝ステージ2「チョコレート火山」が解放</strong>。ここまでを実プレイと同じ手順・同じ結果で、プレイ時間${esc(last.t)}ぶんを短時間で通した。<strong>この放置ベースの収録が忠実に届く範囲はステージ2まで</strong>——ステージ3以降は本来「能動生産(タップ/会心/金/討伐)で毎秒生産を高速に伸ばす」プレイが要り、放置生産一辺倒のこの機構では再現しきれない(＝ゲームの限界でなく収録手法の限界・別途モデル化が要る)。各操作の実画面で「作りとして気持ちいいか」を確認するためのもの。</footer>
</div>`;
const css=`
:root{
